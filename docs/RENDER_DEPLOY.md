...
`SECRET_KEY` is generated by Render. The Blueprint uses
`autoDeployTrigger: commit` so pushes to `main` deploy automatically. Use a
`[skip render]` commit marker only for changes that should not deploy.
