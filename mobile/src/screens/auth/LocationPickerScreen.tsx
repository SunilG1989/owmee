...
// SPRINT8_PHASE2_GEMINI_V2_LOC_FIX — 4s timeout + nearestCity fallback so
// indoor GPS or slow Nominatim doesn't hang the whole login flow.
const INDIAN_CITIES_FOR_FALLBACK = [
...
function nearestIndianCity(lat: number, lng: number): { name: string; lat: number; lng: number } {
  let best = INDIAN_CITIES_FOR_FALLBACK[0], min = Infinity;
  for (const c of INDIAN_CITIES_FOR_FALLBACK) {
...
async function reverseGeocode(lat: number, lng: number): Promise<OwmeeLocation | null> {
  // Network call with 4s budget. On timeout, fall back to pure-math
  // nearest-city so the user can still proceed.
  ...
  try {
    ...
    clearTimeout(timer);
    console.warn('reverseGeocode timed out / failed, using nearestCity fallback', e);
    const city = nearestIndianCity(lat, lng);
    return {
      city: city.name,
      locality: '',
      pincode: '',
...
async function searchPlaces(query: string): Promise<NominatimResult[]> {
  ...
  // SPRINT8_PHASE2_GEMINI_V2_LOC_FIX — 4s timeout
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), 4000);
  try {
    ...
    clearTimeout(timer);
    console.warn('searchPlaces failed / timed out', e);
    return [];
  }
}
  ...
  const detectGPS = useCallback(async () => {
    ...
    try {
      ...
      Geolocation.getCurrentPosition(
        ...
        { enableHighAccuracy: false, timeout: 15000, maximumAge: 300000 } // SPRINT8_PHASE2_GEMINI_V2_LOC_FIX,
      );
    } catch (e) {
  ...
  const handleConfirm = async (final: OwmeeLocation) => {
    ...
    try {
      await Profile.setLocation({
        lat: final.lat,
        lng: final.lng,
        display_name: final.area || final.city || 'My location',
        full_address: final.fullAddress || '',
        city: final.city,
        state: final.state || '',
