...
from app.modules.settlement.jobs import run_settlement_sweeper
...
async def main():
    ...
    settlement_task = asyncio.create_task(run_settlement_sweeper())
    transaction_ops_alert_task = asyncio.create_task(run_transaction_ops_alert_sweeper())
    try:
        await worker.run()
    finally:
        for task in (media_cleanup_task, ai_draft_task, payment_timeout_task, settlement_task, transaction_ops_alert_task):
            task.cancel()
        await asyncio.gather(
            media_cleanup_task,
            ai_draft_task,
