...
class DirectAcquisitionBooking(Base, TimestampMixin):
    ...
    arrival_otp_expires_at = Column(DateTime(timezone=True), nullable=True)
    arrival_otp_attempts = Column(Integer, nullable=False, default=0)
    final_acceptance_otp_hash = Column(String(128), nullable=True)
    final_acceptance_otp_expires_at = Column(DateTime(timezone=True), nullable=True)
    final_acceptance_otp_attempts = Column(Integer, nullable=False, default=0)
    ...
    fe_started_at = Column(DateTime(timezone=True), nullable=True)
    fe_arrived_at = Column(DateTime(timezone=True), nullable=True)
    fe_start_location = Column(JSONB, nullable=False, default=dict)
    fe_arrival_location = Column(JSONB, nullable=False, default=dict)
    seller_verified_location = Column(JSONB, nullable=False, default=dict)
    seller_final_acceptance_location = Column(JSONB, nullable=False, default=dict)
    ...
    payout_ready_by_fe_id = Column(UUID(as_uuid=True), ForeignKey("field_executives.id", ondelete="SET NULL"), nullable=True)
    payout_status = Column(String(32), nullable=False, default="not_started")
    payout_reference_id = Column(String(100), nullable=True)
    payout_processed_by_admin_id = Column(UUID(as_uuid=True), nullable=True)
    payout_failure_reason = Column(Text, nullable=True)
    ...
    warehouse_received_at = Column(DateTime(timezone=True), nullable=True)
    warehouse_received_by_admin_id = Column(UUID(as_uuid=True), nullable=True)
    warehouse_receipt_code = Column(String(80), nullable=True)
    warehouse_receipt_notes = Column(Text, nullable=True)
...
class AcquisitionItem(Base, TimestampMixin):
    ...
    qc_evidence_manifest = Column(JSONB, nullable=False, default=dict)
    qc_notes = Column(Text, nullable=True)
    reject_evidence_photos = Column(JSONB, nullable=False, default=list)
    custody_seal_code = Column(String(80), nullable=True)
    warehouse_status = Column(String(32), nullable=False, default="pending")
