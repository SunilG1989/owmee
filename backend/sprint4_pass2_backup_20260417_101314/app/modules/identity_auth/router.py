...
def _issue_access_token(user, session_id: str) -> str:
    """Single source of truth for access token creation — always includes tri-state claims."""
...
async def verify_otp(body: VerifyOTPRequest, db: DBSession):
    ...
    access_token = _issue_access_token(user, session_id)
...
async def refresh_token(body: RefreshRequest, db: DBSession):
    ...
    new_access = _issue_access_token(user, payload["session_id"])
