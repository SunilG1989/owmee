...
def create_access_token(
    ...
    seller_tier). The existing `tier` and `kyc_status` claims are preserved
    for backward compatibility; callers that haven't been updated yet will
    still work — the new claims default to values inferred from Sprint 3
    state, and the dependencies layer does the same inference.
    """
